...
<div class="grid gap-6 md:grid-cols-2">
    ...
    <div>
        ...
        <input id="salesforce_id" name="salesforce_id" type="text" value="{{ old('salesforce_id', $dealership?->salesforce_id) }}"
            class="w-full rounded-2xl border border-gray-300 px-4 py-3 text-sm text-brand-secondary outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20">
    </div>
</div>
...
<div class="grid gap-6 md:grid-cols-2">
    <div>
        ...
        <input id="google_maps_url" name="google_maps_url" type="url" value="{{ old('google_maps_url', $dealership?->google_maps_url) }}"
            class="w-full rounded-2xl border border-gray-300 px-4 py-3 text-sm text-brand-secondary outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20">
    </div>

    <div>
        <label for="reviews_url" class="mb-2 block pl-2 text-sm font-medium text-brand-secondary">URL de reseñas</label>
        <input id="reviews_url" name="reviews_url" type="url" value="{{ old('reviews_url', $dealership?->reviews_url) }}"
            class="w-full rounded-2xl border border-gray-300 px-4 py-3 text-sm text-brand-secondary outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20">
    </div>
</div>
    ...
    <div class="rounded-2xl border border-brand-secondary/10 bg-slate-50 p-4">
        ...
        <div class="mt-4 flex items-center gap-4">
            ...
            <div class="text-sm text-brand-secondary/70">
                ...
                <p>{{ old('salesforce_id', $dealership?->salesforce_id ?? 'Sin ID de Salesforce') }}</p>
            </div>
        </div>
    </div>
</div>
