...
@section('content')
    <main class="mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-6">
        <section class="rounded-3xl border border-brand-secondary/10 bg-white p-6 shadow-sm">
            <div class="mb-8">
                <h1 class="text-2xl font-bold tracking-tight text-brand-secondary">Editar delegación</h1>
                <p class="mt-2 text-sm text-brand-secondary/70">Actualiza los datos de la delegación seleccionada.</p>
            </div>
            ...
            <form method="POST" action="{{ route('dealerships.update', $dealership) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                @include('dealerships._form', ['dealership' => $dealership])
