    ...
    <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 text-3xl font-semibold text-brand-secondary md:text-4xl">Logs de herramientas de tickets</h1>
                <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                    Aqui puedes revisar las altas, ediciones y eliminaciones de herramientas con su fecha, hora y la persona que realizo la gestión.
                </p>
            </div>

            <a href="{{ route('admin.ticket-tool-logs.export', request()->only(['action', 'date_from', 'date_to', 'actor'])) }}"
        ...
        <form method="GET" action="{{ route('admin.ticket-tool-logs.index') }}" data-logs-filter-form
            ...
            <div class="flex flex-col gap-4">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-stretch xl:justify-between">
                    <div class="grid flex-1 gap-3 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)]">
                        <div class="grid gap-3">
                            <div class="relative rounded-2xl border border-brand-secondary/10 bg-white px-4 py-3">
                                ...
                                <select name="action" class="absolute inset-0 z-10 h-full w-full cursor-pointer appearance-none rounded-2xl opacity-0 outline-none">
                            ...
                            <div class="relative rounded-2xl border border-brand-secondary/10 bg-white px-4 py-3">
                                ...
                                <select name="actor" class="absolute inset-0 z-10 h-full w-full cursor-pointer appearance-none rounded-2xl opacity-0 outline-none">
                        ...
                        <div class="grid min-h-[9.5rem] grid-rows-[auto_1fr] rounded-2xl border {{ $hasActiveRange ? 'border-brand-primary/25 bg-brand-primary/10' : 'border-brand-secondary/10 bg-white' }} px-4 py-3 transition">
                            ...
                            <div class="mt-2 grid h-full gap-2 sm:grid-cols-2">
                                <button type="button" data-date-trigger="from"
                                    ...
                                    <input type="date" name="date_from" value="{{ $dateFrom }}" data-date-input="from" class="pointer-events-none absolute inset-0 opacity-0" tabindex="-1">
                                </button>
                                ...
                                <button type="button" data-date-trigger="to"
                                    ...
                                    <input type="date" name="date_to" value="{{ $dateTo }}" data-date-input="to" class="pointer-events-none absolute inset-0 opacity-0" tabindex="-1">
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-3 xl:min-w-[12rem] xl:grid-cols-1">
                        ...
                        <a href="{{ route('admin.ticket-tool-logs.index') }}" data-logs-reset
            ...
            <table class="min-w-full divide-y divide-brand-secondary/10">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Herramienta afectada</th>
                        ...
                        <tr class="align-top">
                            ...
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <div class="flex items-center gap-3">
                                    <span class="h-4 w-4 rounded-full ring-2 ring-white shadow-sm" style="background-color: {{ $log->target_color }}"></span>
                                    <div>
                                        <p class="font-semibold">{{ $log->target_name }}</p>
                                        <p class="mt-1 text-brand-secondary/65">{{ $log->target_color ?: 'Sin color' }}</p>
                                    </div>
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-sm text-brand-secondary/65">
                                Todavia no hay logs de herramientas para los filtros seleccionados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
