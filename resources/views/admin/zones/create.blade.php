        ...
        <section class="rounded-[2rem] border border-brand-secondary/10 bg-white p-6 shadow-sm md:p-8">
            <div class="flex flex-col gap-6">
                <div class="max-w-3xl">
                    ...
                    <h1 class="mt-3 text-3xl font-semibold text-brand-secondary md:text-4xl">Crear zona</h1>
                    <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                        Define una nueva zona y asigna las delegaciones que quieras, respetando que cada delegación solo pueda pertenecer a una zona.
                    </p>
                </div>

                <form method="POST" action="{{ route('admin.zones.store') }}" class="space-y-8" data-zone-form-root>
                    @csrf
                    @include('admin.zones._form', ['zone' => null, 'availableDealerships' => $availableDealerships])
