    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-brand-secondary/10">
                ...
                <tr>
                    @foreach ([
                        ...
                        <th class="px-6 py-4 text-left">
                            <a href="{{ route('admin.chat-groups.index', array_merge($persistedQuery, ['sort' => $column, 'direction' => $sortDirection($column, $sort, $direction)])) }}"
            ...
            <tbody class="divide-y divide-brand-secondary/10 bg-white">
                    ...
                    <tr class="align-top transition hover:bg-brand-secondary/5">
                        ...
                        <td class="px-6 py-4 text-sm text-brand-secondary/80">
                            <span class="inline-flex rounded-full bg-brand-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-brand-primary">
                                {{ $group->participants_count ?? $group->participants->count() }} usuarios
                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm text-brand-secondary/80">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.chat-groups.edit', $group) }}" class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-xl border border-brand-secondary/15 bg-white text-brand-secondary transition hover:bg-brand-secondary/5" title="Editar grupo" aria-label="Editar grupo">
                                ...
                                <form method="POST" action="{{ route('admin.chat-groups.destroy', $group) }}" onsubmit="return confirm('Seguro que quieres eliminar este grupo?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-xl border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100 hover:text-red-700" title="Eliminar grupo" aria-label="Eliminar grupo">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-sm text-brand-secondary/70">No hay grupos registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
