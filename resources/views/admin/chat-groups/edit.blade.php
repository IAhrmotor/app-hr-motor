    ...
    <main class="mx-auto max-w-7xl px-6 py-10">
        <section class="rounded-[2rem] border border-brand-secondary/10 bg-white p-6 shadow-sm md:p-8">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 text-3xl font-semibold text-brand-secondary md:text-4xl">Editar grupo del chat</h1>
                <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                    Actualiza el nombre y la composición del grupo seleccionado.
                </p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.chat-groups.update', $group) }}" class="mt-8 space-y-8" data-chat-groups-root>
                @csrf
                @method('PUT')

                @include('admin.chat-groups._form', ['group' => $group])
