    ...
    <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 text-3xl font-semibold text-brand-secondary md:text-4xl">Política de aceptación</h1>
                <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                    Consulta quién ha aceptado la política vigente del chat corporativo y descarga el histórico en CSV.
                </p>
            </div>

            <a href="{{ route('admin.policy-acceptance-logs.export', request()->only(['date_from', 'date_to', 'user'])) }}"
        ...
        <form method="GET" action="{{ route('admin.policy-acceptance-logs.index') }}" data-logs-filter-form
            ...
            <div class="flex flex-col gap-4">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-stretch xl:justify-between">
                    ...
                    <div class="grid gap-3 xl:min-w-[12rem] xl:grid-cols-1">
                        ...
                        <a href="{{ route('admin.policy-acceptance-logs.index') }}" data-logs-reset
            ...
            <table class="min-w-full divide-y divide-brand-secondary/10">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Email</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Versión</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">IP</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Origen</th>
                    </tr>
                </thead>
                    ...
                    @forelse ($logs as $log)
                        <tr class="align-top">
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-semibold">{{ $log->accepted_at?->format('d/m/Y') }}</p>
                                <p class="mt-1 text-brand-secondary/65">{{ $log->accepted_at?->format('H:i:s') }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-semibold">{{ $log->user?->name ?? 'Usuario eliminado' }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="break-all">{{ $log->user_email }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                            ...
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="break-all">{{ $log->ip_address ?: 'N/D' }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-medium">{{ $log->source }}</p>
                                <p class="mt-1 break-all text-brand-secondary/65">{{ $log->user_agent ?: 'N/D' }}</p>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-sm text-brand-secondary/65">
                                Todavía no hay aceptaciones de política para los filtros seleccionados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
