    ...
    <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 text-3xl font-semibold text-brand-secondary md:text-4xl">Borrado chats</h1>
                <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                    Consulta las ejecuciones diarias de borrado automático de mensajes de chat y descarga el histórico en CSV.
                </p>
            </div>

            <a href="{{ route('admin.chat-retention-logs.export', request()->only(['date_from', 'date_to', 'user'])) }}"
        ...
        <form method="GET" action="{{ route('admin.chat-retention-logs.index') }}" data-logs-filter-form
                ...
                <div class="flex flex-col gap-3 xl:flex-row xl:items-stretch xl:justify-between">
                    <div class="grid flex-1 gap-3 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)]">
                        <div class="relative rounded-2xl border border-brand-secondary/10 bg-white px-4 py-3">
                            <div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-secondary/45">Usuario</div>
                            <div data-display-user class="pointer-events-none pr-10 text-base font-semibold text-brand-secondary">
                                {{ optional($users->firstWhere('id', $userId))->name ?? 'Todos los usuarios' }}
                            </div>
                            <select name="user"
                    ...
                    <div class="grid gap-3 xl:min-w-[12rem] xl:grid-cols-1">
                        ...
                        <a href="{{ route('admin.chat-retention-logs.index') }}" data-logs-reset
            ...
            <table class="min-w-full divide-y divide-brand-secondary/10">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Borrados</th>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Errores</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-brand-secondary/60">Corte</th>
                    </tr>
                </thead>
                    ...
                    @forelse ($logs as $log)
                        <tr class="align-top">
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-semibold">{{ $log->executed_at?->format('d/m/Y') }}</p>
                                <p class="mt-1 text-brand-secondary/65">{{ $log->executed_at?->format('H:i:s') }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] {{ $log->status === 'success' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                    {{ $log->status_label }}
                                </span>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-semibold">{{ $log->deleted_count }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-medium">{{ filled($log->affected_users) ? collect($log->affected_users)->map(fn ($user) => is_array($user) ? (($user['name'] ?? 'Usuario') . (filled($user['count'] ?? null) ? ' (' . $user['count'] . ')' : '')) : (string) $user)->implode(' | ') : 'Sin usuarios afectados' }}</p>
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                @if ($log->error_count > 0)
                                    <p class="font-semibold text-rose-600">{{ $log->error_count }} error{{ $log->error_count === 1 ? '' : 'es' }}</p>
                                    @if ($log->error_summary)
                                        <p class="mt-1 text-brand-secondary/65">{{ $log->error_summary }}</p>
                                    @endif
                                @else
                                    <span class="text-brand-secondary/60">Sin errores</span>
                                @endif
                            </td>
                            <td class="px-6 py-5 text-sm text-brand-secondary">
                                <p class="font-medium">{{ $log->cutoff?->format('d/m/Y H:i') }}</p>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-sm text-brand-secondary/65">
                                Todavía no hay borrados de chats para los filtros seleccionados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
