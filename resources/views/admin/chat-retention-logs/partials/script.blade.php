<script>
    window.initAdminLogsFilters = function () {
        ...
        const userDisplay = form.querySelector('[data-display-user]');
        ...
        const updateSelectDisplay = () => {
            ...
            userDisplay.textContent = userSelect.options[userSelect.selectedIndex]?.text || 'Todos los usuarios';
        };
        ...
        userSelect?.parentElement?.addEventListener('click', (event) => {
            if (event.target !== userSelect) {
                openSelect(userSelect);
            }
        });
