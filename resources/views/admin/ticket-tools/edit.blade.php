    ...
    <main class="mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-8 lg:px-8">
        <section class="rounded-[2rem] border border-brand-secondary/10 bg-white p-6 shadow-sm md:p-8">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-3 text-3xl font-bold tracking-tight text-brand-secondary md:text-4xl">Editar tipo de incidencia</h1>
                <p class="mt-3 text-sm leading-6 text-brand-secondary/70 md:text-base">
                    Ajusta el nombre o el color del tipo de incidencia y revisa el resultado antes de guardarlo.
                </p>
            </div>
            ...
            <div class="mt-8">
                @include('admin.ticket-tools._form', [
                    'tool' => $tool,
                    'action' => route('admin.ticket-tools.update', $tool),
                    'method' => 'PUT',
                    'submitLabel' => 'Guardar cambios',
                ])
            </div>

            <form method="POST" action="{{ route('admin.ticket-tools.destroy', $tool) }}" class="mt-8">
                @csrf
                @method('DELETE')
                <button type="submit"
