        ...
        <section class="rounded-3xl border border-brand-secondary/10 bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('users.store') }}" class="space-y-8">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div id="dealership-wrapper" @class(['rounded-2xl border border-brand-primary/15 bg-brand-primary/5 px-4 py-4', 'hidden' => ! $showDealershipField])>
                        <label for="dealership" class="mb-2 block text-sm font-medium text-brand-secondary">Delegación</label>
                        <div class="relative">
                            <select id="dealership" name="dealership_id" @required($showDealershipField)
                        ...
                        <p class="mt-2 text-xs text-brand-secondary/60">Obligatoria para usuarios con rol comercial.</p>
                    </div>

                    <div id="salesforce-user-id-wrapper" @class(['rounded-2xl border border-brand-primary/15 bg-brand-primary/5 px-4 py-4', 'hidden' => ! $showSalesforceField])>
                        <label for="salesforce_user_id" class="mb-2 block text-sm font-medium text-brand-secondary">ID de usuario en Salesforce</label>
                        <input id="salesforce_user_id" name="salesforce_user_id" type="text" value="{{ old('salesforce_user_id') }}" @required($showSalesforceField)
                            class="w-full rounded-2xl border border-gray-300 px-4 py-3 text-sm text-brand-secondary outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20">
                        <p class="mt-2 text-xs text-brand-secondary/60">Este campo solo aplica a usuarios con rol comercial.</p>
                    </div>
                </div>
