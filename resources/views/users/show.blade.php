        ...
        <section class="rounded-3xl border border-brand-secondary/10 bg-white p-6 shadow-sm md:p-8">
            <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                <div class="flex items-center gap-5">
                    <img src="{{ $user->avatar_url }}" alt="Avatar de {{ $user->name }}" class="h-24 w-24 rounded-full object-cover ring-2 ring-brand-primary/10">
                ...
                <dl class="mt-5 grid gap-5 text-sm md:grid-cols-3">
                    ...
                    @if ($user->resolved_dealership_name)
                        <div>
                            ...
                            <dd class="mt-1 font-semibold text-brand-secondary">{{ $user->resolved_dealership_name }}</dd>
                        </div>
                    @endif
                </dl>
            </section>
