                    ...
                    <div class="mt-5 flex flex-wrap items-start justify-between gap-4">
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-3 text-sm text-brand-secondary/65">
                                <div class="flex items-center gap-3 rounded-2xl border border-brand-secondary/10 bg-slate-50 px-3 py-2">
                                    <img src="{{ $thread->creator->avatar_url }}" alt="Avatar de {{ $thread->creator->name }}" class="h-10 w-10 rounded-xl object-cover">
                                    <div>
                                        <div class="flex flex-wrap items-center gap-2">
                                            <p class="font-semibold {{ $isThreadCreatorStoreManager ? 'text-amber-700' : 'text-brand-secondary' }}">{{ $thread->creator->name }}</p>
                            ...
                            <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                @foreach ($thread->attachments as $attachment)
                                    <a href="{{ $attachment->image_url }}" target="_blank" rel="noopener noreferrer"
                                        class="overflow-hidden rounded-2xl border border-brand-secondary/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                                        <img src="{{ $attachment->image_url }}" alt="Adjunto del hilo" class="h-52 w-full object-cover">
                                    </a>
                                @endforeach
                            </div>
                        @endif
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                                                <p class="font-semibold {{ $isReplyAuthorStoreManager ? 'text-amber-700' : 'text-brand-secondary' }}">{{ $reply->author->name }}</p>
                                            ...
                                            @if ($reply->attachments->isNotEmpty())
                                                <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                                    @foreach ($reply->attachments as $attachment)
                                                        <a href="{{ $attachment->image_url }}" target="_blank" rel="noopener noreferrer"
                                                            class="overflow-hidden rounded-2xl border border-brand-secondary/10 bg-slate-50 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                                                            <img src="{{ $attachment->image_url }}" alt="Adjunto de la respuesta" class="h-44 w-full object-cover">
                                                        </a>
                                                    @endforeach
                                                </div>
                                            @endif
