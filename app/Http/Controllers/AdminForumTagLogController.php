...
class AdminForumTagLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (! Schema::hasTable('forum_tag_activity_logs')) {
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        if (! Schema::hasTable('forum_tag_activity_logs')) {
            return response()->streamDownload(function (): void {
                ...
                fclose($output);
            }, 'logs-tags-pendiente-migracion.csv', [
        ...
        $filename = $action
            ? "logs-tags-{$action}-{$timestamp}.csv"
            : "logs-tags-{$timestamp}.csv";
    ...
    {
        $allowedActions = [
            ForumTagActivityLog::ACTION_CREATED,
            ForumTagActivityLog::ACTION_UPDATED,
            ForumTagActivityLog::ACTION_DELETED,
        ];
    ...
    {
        return ForumTagActivityLog::query()
    ...
    private function renderIndexResponse(Request $request, array $data)
    {
        if ($request->ajax()) {
            return response()->json([
                'html' => view('admin.forum-tag-logs.partials.content', $data)->render(),
            ]);
        }

        return view('admin.forum-tag-logs.index', $data);
    }
}
