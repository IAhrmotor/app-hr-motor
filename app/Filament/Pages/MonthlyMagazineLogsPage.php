...
class MonthlyMagazineLogsPage extends Page implements HasTable
{
    use InteractsWithTable;

    protected static bool $shouldRegisterNavigation = false;

    protected static ?string $slug = 'revista/logs';

    protected static ?string $breadcrumb = 'Logs de revista';

    protected static ?string $title = 'Logs de revista';
    ...
    {
        return $table
            ->query(fn (): Builder => MonthlyMagazineActivityLog::query())
                ...
                Filter::make('action')
                    ...
                    ->form([
                        Select::make('value')
                            ->label('Acción')
                            ->options([
                                MonthlyMagazineActivityLog::ACTION_CREATED => 'Alta',
                                MonthlyMagazineActivityLog::ACTION_UPDATED => 'Edición',
                                MonthlyMagazineActivityLog::ACTION_DELETED => 'Eliminación',
                            ])
            ...
            ->columns([
                ...
                TextColumn::make('actor_name')
                    ->label('Gestionado por')
                    ->state(fn (MonthlyMagazineActivityLog $record): string => $record->actor_name)
                    ->description(fn (MonthlyMagazineActivityLog $record): ?string => $record->actor_email)
                ...
                TextColumn::make('changes')
                    ->label('Detalle')
                    ->state(function (MonthlyMagazineActivityLog $record): string {
    ...
    {
        return route('backoffice.monthly-magazine-logs.export', array_filter([
    ...
    protected function formatChanges(MonthlyMagazineActivityLog $record): string
    {
        $changes = MonthlyMagazineActivityLogWriter::normalizeChanges($record->changes ?? []);
