...
class CommercialCommissionsInterfaceTest extends TestCase
{
    ...
    public function test_complete_breakdown_is_rendered_without_technical_fields_or_economic_status(): void
    {
        ...
        $response = $this->actingAs($commercial)
            ...
            ->assertSee('788,96 €')
            ->assertSee('+')
            ->assertSeeInOrder(['−', 'Penalización por descuento'])
            ->assertSeeInOrder(['−', 'Penalización de garantías'])
            ->assertSeeInOrder(['−', 'Penalización de reseñas'])
            ->assertSeeInOrder(['−', 'Penalización de financiación'])
            ->assertSeeInOrder(['−', 'Cancelación de financiación'])
    ...
    public function test_appraiser_uses_only_appraiser_settlement_lines(): void
    {
        ...
        $this->actingAs($appraiser)
            ->get(route('profile.show', ['month' => '2026-08']))
            ->assertOk()
            ->assertSeeInOrder(['+', 'Compras gestionadas'])
            ->assertSeeInOrder(['+', 'Ventas gestionadas'])
            ->assertSeeInOrder(['+', 'Comisión de financiación'])
            ->assertSeeInOrder(['+', 'Incentivo por velocidad'])
            ->assertSeeInOrder(['−', 'Cancelación de financiación'])
            ->assertSee('230,00 €')
    ...
    public function test_real_commercial_payload_explains_official_final_commission_without_duplicate_amounts(): void
    {
        ...
        $response = $this->actingAs($commercial)
            ...
            ->assertSeeInOrder(['−', 'Penalización por descuento'])
            ->assertSee('19,50 €')
            ->assertSeeInOrder(['+', 'Tasaciones y cambios'])
            ->assertSee('60,00 €')
            ->assertSee('Subtotal de prima')
            ->assertSee('490,50 €')
            ->assertSeeInOrder(['−', 'Ajuste por tramo de entregas'])
            ->assertSee('86,10 €')
            ...
            ->assertSee('Prima después de penalizaciones')
            ->assertSee('363,96 €')
            ->assertSeeInOrder(['+', 'Comisión de producto financiero'])
            ->assertSee('354,20 €')
            ->assertSeeInOrder(['+', 'Comisión de producto de garantía'])
            ->assertSee('70,80 €')
    ...
    public function test_partial_and_legacy_breakdowns_hide_only_missing_values(): void
    {
        ...
        $this->actingAs($commercial)
            ...
            ->assertOk()
            ->assertSee('0,00 €')
            ->assertDontSee('Ajuste por tramo de entregas')
